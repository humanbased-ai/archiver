<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
          v-hasPermi="['system:role:add']">add</el-button>
      </el-col>
      <!-- <el-col :span="1.5">
    ...
    <el-table v-loading="loading" :data="roleList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="roleID" prop="id" width="220" />
      ...
      <el-table-column label="Operation" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope" v-if="scope.row.roleId !== 1">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
            v-hasPermi="['system:role:edit']">change</el-button>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item prop="roleKey">
          <span slot="label">
            <el-tooltip content="The permission characters defined in the controller, such as @ PreAuthorize (` @ ss. hasRole ('admin ') `)" placement="top">
        ...
        <el-form-item label="Menu permissions">
          ...
          <el-checkbox v-model="form.menuCheckStrictly" @change="handleCheckedTreeConnect($event, 'menu')">Father son linkage
          </el-checkbox>
          <el-tree class="tree-border" :data="menuOptions" show-checkbox ref="menu" node-key="value"
            :check-strictly="!form.menuCheckStrictly" empty-text="loading" :props="defaultProps"></el-tree>
        </el-form-item>
    ...
    <el-dialog :title="title" :visible.sync="openDataScope" width="500px" append-to-body>
      <el-form :model="form" label-width="80px">
        ...
        <el-form-item label="Data permissions" v-show="form.dataScope == 2">
          ...
          <el-checkbox v-model="form.deptCheckStrictly" @change="handleCheckedTreeConnect($event, 'dept')">Father son linkage
          </el-checkbox>
          <el-tree class="tree-border" :data="deptOptions" show-checkbox default-expand-all ref="dept" node-key="value"
            :check-strictly="!form.deptCheckStrictly" empty-text="loading" :props="defaultProps"></el-tree>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
...
export default {
  ...
  data() {
    return {
      ...
      dataScopeOptions: [
        ...
        {
          value: "3",
          label: "Data permissions for this department"
        },
        {
          value: "4",
          label: "Data permissions for this department and below"
        },
        {
          value: "5",
          label: "Only have personal data permissions"
        }
      ],
      // 菜单列表
      menuOptions: [],
      ...
      rules: {
        ...
        roleSort: [
          { required: true, message: "The order of roles cannot be empty", trigger: "blur" }
        ]
      }
    };
  },
  ...
  methods: {
    ...
    handleStatusChange(row) {
      let text = row.status === 0 ? "start-up" : "shut down";
      this.$modal.confirm('sure to "' + text + '""' + row.roleName + '"？').then(function () {
    ...
    handleCheckedTreeExpand(value, type) {
      if (type == 'menu') {
        ...
        for (let i = 0; i < treeList.length; i++) {
          this.$refs.menu.store.nodesMap[treeList[i].value].expanded = value;
        }
      } else if (type == 'dept') {
        let treeList = this.deptOptions;
        for (let i = 0; i < treeList.length; i++) {
          this.$refs.dept.store.nodesMap[treeList[i].value].expanded = value;
        }
      }
    },
    // 树权限（全选/全不选）
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('are you sure delete the item "' + roleIds + '"？').then(function () {
