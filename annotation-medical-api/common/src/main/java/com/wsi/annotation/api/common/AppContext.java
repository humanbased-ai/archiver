package com.wsi.annotation.api.common;

/**
 * 
 * @ClassName: AppContext
 * @Description: TODO(这里用一句话描述这个类的作用)
 * @author 252956
 * @date 2019年10月21日 下午4:52:33
 *
 */
public class AppContext {

	public static final String TOKEN = "IM-Manage-Token";

	public static final String Department_Key = "IM_MANAGE_department_key";

	public static final String USER_INFO = "IM_MANAGE_user_info_";

	// ok
	public static final int CODE_20000 = 20000;
	// error
	public static final int CODE_50000 = 50000;
	// Illegal token
	public static final int CODE_50001 = 50001;
	// no permission
	public static final int CODE_50002 = 50002;
	// DemoSystem
	public static final int CODE_50003 = 50003;
	// service reject request
	public static final int CODE_50004 = 50004;

}
