...
public class SecurityUtils
{
     ...
     * 获取用户
     **/
    public static BasicUser getCookieUser()
    {
        ...
        try
        {
            ...
            if (userCookie.size() > 0) {
                JSONObject sessionJson = JSONObject.parseObject(URLDecoder.decode(userCookie.get(0).getValue(), "UTF-8"));
                user.setId(sessionJson.getString("id"));
                user.setUserName(sessionJson.getString("username"));
                user.setName(sessionJson.getString("customerName"));
                user.setLanguage(sessionJson.getString("language"));
            }
        }
        catch (Exception e)
        {
