...
function createRowHtml(data,opt,index){
    ...
    if(data!=null){
        ...
        var idHtml = '<a href="'+data.url+'" target="_blank">'+data.content+'</a>'

        //html +='<td>'+idHtml+'</td>';
        html +='<td>'+getInputHtml('content',data.content,'width:80px;','text')+'</td>';

        var image_html = ''
        var image_urls = data.image_urls;
        if(image_urls!=null && image_urls.length>0){
            for(var k=0;k<image_urls.length;k++){
                var image_url = image_urls[k]
                image_html +='<img src="'+image_url+'" class="zoomable-image" style="">'
            }
        }

        html +='<td>'+image_html+'</td>';
        html +='<td>'+getInputHtml('status',post_status_map[data.status],'width:60px;','text')+'</td>';


        html +='<td>'+getInputHtml('ip',data.ip,'width:80px;','text')+'</td>';
