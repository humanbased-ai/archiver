"""
测试阿里云千问(Qwen)模型的模块。
该模块包含了用于测试阿里云千问系列模型的功能，支持文本到文本的测试。
"""
...
# 导入测试版本的 chat_qwen_model 函数
from service.ct.tests.chat_service import chat_qwen_model, user_queues
# 导入模型配置
...
async def test_models(api_key, models=None, prompt_type="basic", org=None, input_type=None, output_type=None, llm_only=False, multimodal_only=False):
    ...
    if models:
        ...
        if org:
            ...
            model_names = get_llm_models(ModelOrg.QWEN.value)
    elif multimodal_only:
        # 获取多模态大语言模型
        if org:
            model_names = get_multimodal_llm_models(org)
        else:
            model_names = get_multimodal_llm_models(ModelOrg.QWEN.value)
    elif org:
        # 根据组织筛选模型
    ...
    for model in model_names:
        ...
        try:
            ...
            # 只有千问模型才使用 chat_qwen_model 函数
            if model_data["org"] == ModelOrg.QWEN.value:
                print(f"发送请求到 {model}...")
                result = await chat_qwen_model(test_uid, model_data, prompt, f"test_{model}")
                print(f"响应: {result}")
            else:
                print(f"注意: 当前只支持测试千问模型。模型 {model} 属于 {model_data['org']} 组织，需要实现相应的测试函数。")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='测试阿里云千问模型')
    parser.add_argument('--api-key', type=str, help='阿里云千问 API 密钥')
    parser.add_argument('--models', type=str, nargs='+', help='要测试的模型列表，例如 "qwen-max qwen-plus"')
    ...
    parser.add_argument('--org', type=str, choices=[org.value for org in ModelOrg], default="qwen", 
                        help='按组织筛选模型: openai, claude, gemini, qwen')
    ...
    if not api_key:
        api_key = read_env_file('QWEN_API_KEY')
    ...
    if not api_key:
        print("错误: 未提供阿里云千问 API 密钥。请使用 --api-key 提供或设置 QWEN_API_KEY 环境变量。")
        sys.exit(1)
