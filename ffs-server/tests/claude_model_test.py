"""
测试 Claude 模型的模块。
该模块包含了用于测试 Anthropic Claude 系列模型的功能，支持文本到文本和图像到文本的测试。
"""
...
from service.ct.tests.chat_service import chat_openai_model, user_queues
# 导入模型配置
...
async def test_models(api_key, models=None, prompt_type="basic", org=None, input_type=None, output_type=None, llm_only=False, multimodal_only=False):
    ...
    if models:
        ...
        if org:
            ...
            model_names = get_llm_models(ModelOrg.CLAUDE.value)
    elif multimodal_only:
        # 获取多模态大语言模型
        if org:
            model_names = get_multimodal_llm_models(org)
        else:
            model_names = get_multimodal_llm_models(ModelOrg.CLAUDE.value)
    elif org:
        # 根据组织筛选模型
    ...
    for model in model_names:
        ...
        try:
            ...
            # 只有 Claude 模型才使用 chat_claude_model 函数
            if model_data["org"] == ModelOrg.CLAUDE.value:
                print(f"发送请求到 {model}...")
                result = await chat_openai_model(test_uid, model_data, prompt, f"test_{model}")
                print(f"响应: {result}")
            else:
                print(f"注意: 当前只支持测试 Claude 模型。模型 {model} 属于 {model_data['org']} 组织，需要实现相应的测试函数。")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='测试 Claude 模型')
    parser.add_argument('--api-key', type=str, help='Anthropic API 密钥')
    parser.add_argument('--models', type=str, nargs='+', help='要测试的模型列表，例如 "claude-3-opus-20240229 claude-3-sonnet-20240229"')
    ...
    parser.add_argument('--org', type=str, choices=[org.value for org in ModelOrg], default="claude", 
    ...
    if not api_key:
        api_key = read_env_file('CLAUDE_API_KEY')
    
    # 如果还是没有，尝试从setting.py获取
    if not api_key:
        try:
            from setting import anthropic_api_key
    ...
    if not api_key:
        print("错误: 未提供 Anthropic API 密钥。请使用 --api-key 提供或设置 CLAUDE_API_KEY 环境变量。")
        sys.exit(1)
