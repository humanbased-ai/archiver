"""
测试 DeepSeek 模型的模块。
该模块包含了用于测试 DeepSeek 系列模型的功能，支持文本到文本的测试。
"""
...
# 导入测试版本的 chat_deepseek_model 函数
from service.ct.tests.chat_service import chat_deepseek_model, user_queues
# 导入模型配置
...
async def test_models(api_key, models=None, prompt_type="basic", org=None, input_type=None, output_type=None, llm_only=False, multimodal_only=False):
    ...
    if models:
        ...
        if org:
            ...
            model_names = get_llm_models(ModelOrg.DEEPSEEK.value)
    elif multimodal_only:
        # 获取多模态大语言模型
        if org:
            model_names = get_multimodal_llm_models(org)
        else:
            model_names = get_multimodal_llm_models(ModelOrg.DEEPSEEK.value)
    elif org:
        # 根据组织筛选模型
    ...
    for model in model_names:
        ...
        try:
            ...
            if model_data["org"] == ModelOrg.DEEPSEEK.value:
                print(f"发送请求到 {model}...")
                result = await chat_deepseek_model(test_uid, model_data, prompt, f"test_{model}")
                print(f"响应: {result}")
            else:
                print(f"注意: 当前只支持测试 DeepSeek 模型。模型 {model} 属于 {model_data['org']} 组织，需要实现相应的测试函数。")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='测试 DeepSeek 模型')
    parser.add_argument('--api-key', type=str, help='DeepSeek API 密钥')
    parser.add_argument('--models', type=str, nargs='+', help='要测试的模型列表，例如 "deepseek-chat deepseek-coder"')
    ...
    parser.add_argument('--org', type=str, choices=[org.value for org in ModelOrg], default="deepseek", 
    ...
    if not api_key:
        api_key = read_env_file('DEEPSEEK_API_KEY')
    ...
    if not api_key:
        print("错误: 未提供 DeepSeek API 密钥。请使用 --api-key 提供或设置 DEEPSEEK_API_KEY 环境变量。")
        sys.exit(1)
