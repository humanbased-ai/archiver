...
class BrowserContext:
    ...
    async def get_state(self, use_vision: bool = False, step_info: Optional[AgentStepInfo] = None,
                        agent_id=None) -> BrowserState:
        """Get the current state of the browser"""
        await self._wait_for_page_and_frames_load()
        session = await self.get_session()
        session.cached_state = await self._update_state(use_vision=use_vision, step_info=step_info, agent_id=agent_id)
    ...
    async def _update_state(self, use_vision: bool = False, focus_element: int = -1,
                            step_info: Optional[AgentStepInfo] = None, agent_id=None) -> BrowserState:
        ...
        try:
            ...
            screenshot = None
            if use_vision:
                screenshot_b64, screenshot = await self.take_screenshot()

            # Ensure the folder for the agent exists
            folder_path = f"case-{agent_id}"
            os.makedirs(folder_path, exist_ok=True)  # Create folder if it doesn't exist
            # 生成文件名，取前 16 个字符
            if step_info:
                filename = f"{folder_path}/{step_info.step_number + 1}.png"
                # 保存图片
                with open(filename, "wb") as file:
                    file.write(screenshot)
                logger.warn(f"Screenshot saved as {filename}")
