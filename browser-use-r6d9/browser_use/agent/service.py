...
class Agent:
    ...
    async def step(self, step_info: Optional[AgentStepInfo] = None) -> None:
        ...
        try:
            # print(self.use_vision)
            state = await self.browser_context.get_state(use_vision=self.use_vision, step_info=step_info,
                                                         agent_id=self.agent_id)
            # import copy
            # state_copy = copy.deepcopy(state)  # 避免修改原数据
            # state_copy.screenshot = "[BASE64 DATA OMITTED]"
    ...
    async def get_next_action(self, input_messages: list[BaseMessage],
                              step_info: Optional[AgentStepInfo] = None) -> AgentOutput:
        ...
        # Ensure the folder for the agent exists
        folder_path = f"case-{self.agent_id}"
        os.makedirs(folder_path, exist_ok=True)  # Create folder if it doesn't exist
        # Prepare file paths for input and output based on step number
        input_file_path = os.path.join(folder_path, f"{step_info.step_number + 1}_input.json")
        # Write input messages as string to the input file
        with open(input_file_path, "w") as input_file:
            input_file.write(str(input_messages))  # Convert to string and write to file

        # 提取 'evaluation_previous_goal'
        ss = str(response).split('"evaluation_previous_goal":"')[1].split(" - ")[0]
        output_file_path = os.path.join(folder_path, f"{step_info.step_number + 1}_output_{ss}.json")
        # Write response as string to the output file
        with open(output_file_path, "w") as output_file:
            output_file.write(str(response))  # Convert to string and write to file

        # logger.warn(f"LLM input: {input_messages}")
        # logger.warn(f"LLM response: {response}")
        ...
        key_info_file_path = f'{folder_path}/{step_info.step_number + 1}_output_key_info_{ss}.json'
        self._log_response(parsed, key_info_file_path)
        self.n_steps += 1

        return parsed

    def _log_response(self, response: AgentOutput, key_info_file_path) -> None:
        ...
        response_json = {
            ...
            'action': [a.model_dump_json(exclude_unset=True) for i, a in enumerate(response.action)]
        }
        # 将数据写入 JSON 文件
        with open(key_info_file_path, 'w', encoding='utf-8') as json_file:
    ...
    async def _validate_output(self) -> bool:
        ...
        logger.warning(f"Validate LLM input: {msg}")
        validator = self.llm.with_structured_output(ValidationResult, include_raw=True)
        response: dict[str, Any] = await validator.ainvoke(msg)  # type: ignore
        parsed: ValidationResult = response['parsed']
        logger.warning(f"Validate LLM response: {parsed}")
    ...
    async def _update_action_indices(
        ...
        current_element = HistoryTreeProcessor.find_history_element_in_tree(historical_element,
    ...
    def _create_frame(self, screenshot: str, text: str, step_number: int, width: int = 1200,
