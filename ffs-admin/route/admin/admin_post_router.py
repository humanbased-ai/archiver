...
from service.admin import admin_post_service
...
@router.post("/api/admin/post/page")
@check_login('admin')
async def find_post_page(body: PostParam, request: Request):
    logger.info("查询post列表 {}", body)
    data = await admin_post_service.find_post_page(body)
