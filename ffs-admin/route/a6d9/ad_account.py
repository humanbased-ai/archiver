...
@router.post("/api/ad/account/address/login")
async def login(body: ADAccountParam, request: Request):
    ...
    # 每个用户每分钟10次
    await ralte_limit_utils.rate_limit(request_key=f'api_ad_account_address_register#{address}', limit=20, period=60)

    try:
        # 注册用户
        data = await ad_account_service.login(address_type, address, message, signature)
...
@router.post("/api/ad/account/address/bind")
@check_login('ad')
async def bind(body: ADAccount, request: Request):
    ...
    # 每个用户每分钟10次
    await ralte_limit_utils.rate_limit(request_key=f'api_ad_account_info#{account_id}', limit=20, period=60)
    address_type = body.type
    message = body.message
    signature = body.signature
    address = body.address
    try:
        # 绑定钱包
        data = await ad_account_service.bind(account_id, address_type, address, message, signature)
        content_data = APIResponse(errorcode.SUCCESS, data, "success").set_api_dict()
    except Exception as e:
...
@router.get("/api/ad/account/wallet/info")
@check_login('ad')
async def get_account_wallet(request: Request):
    ...
    account_id = user.get('id', None)
    # 每个用户每分钟10次
    await ralte_limit_utils.rate_limit(request_key=f'api_ad_account_wallet_info#{account_id}', limit=20, period=60)

    try:
        # 查询推文内容，和验证码记录做比较
        data = await ad_account_service.get_account_wallet(account_id)
        content_data = APIResponse(errorcode.SUCCESS, data, "success").set_api_dict()
    except Exception as e:
        logger.exception("check error param = {} {}", '', e)
