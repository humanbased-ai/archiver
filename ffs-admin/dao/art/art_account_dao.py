...
bot_account_colums = f'`id`, `user_id`, `user_name`, `source_type`, `account_code`, `status`, `remarks`, consumer_id, consumer_token, invite_code '


async def get_accounts(status):
    sql = f'select {bot_account_colums} from art_account where deleted=0 and status = "{status}"'
...
async def get_account_by_id(id):
    data = None
    sql = f'select {bot_account_colums} from art_account where deleted=0 and id = "{id}"'
    db_datas = db.sql_to_dict(sql)
    if len(db_datas) > 0:
        data = db_datas[0]
    return data
...
async def get_account_by_invite_code(invite_code):
    data = None
    sql = f'select {bot_account_colums} from art_account where deleted=0 and invite_code = "{invite_code}"'
    db_datas = db.sql_to_dict(sql)
    if len(db_datas) > 0:
        data = db_datas[0]
    return data
...
async def get_account_by_token(account_code):
    data = None
    sql = f'select {bot_account_colums} from art_account where deleted=0 and account_code = "{account_code}"'
    db_datas = db.sql_to_dict(sql)
    if len(db_datas) > 0:
        data = db_datas[0]
    return data
...
async def get_account_by_address(address, source_type="address"):
    data = None
    sql = f'select {bot_account_colums} from art_account where deleted=0 and source_type="{source_type}" and user_id = "{address}"'
    db_datas = db.sql_to_dict(sql)
    if len(db_datas) > 0:
        data = db_datas[0]
    return data
...
async def update_account(data):
    db.update(data, "art_account")


async def add_account(data):
    id = db.insert(data, "art_account", replace=True)
    return id
...
async def save_account(data):
    ...
    logger.info('art_account data = {}', data)


async def get_account_address_list(account_id):
    data = None
    sql = f'select * from art_account_address where deleted=0 and account_id = "{account_id}" '
    db_datas = db.sql_to_dict(sql)
    return db_datas
...
async def get_account_address_by_address(address):
    data = None
    sql = f'select * from art_account_address where deleted=0 and address = "{address}" '
    db_datas = db.sql_to_dict(sql)
    if len(db_datas) > 0:
        data = db_datas[0]
    return data
...
async def update_account_address(data):
    db.update(data, "art_account_address")


async def add_account_address(data):
    db.insert(data, "art_account_address", replace=True)
