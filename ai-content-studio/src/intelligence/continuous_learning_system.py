...
class ContinuousLearningSystem:
    ...
    def add_training_samples(
        ...
        try:
            # 加载现有训练数据
            training_file = self.training_data_dir / f'training_data_{content_type}.json'
            ...
            if existing_samples:
                last_id = max([int(s['id'].split('_')[1]) for s in existing_samples if 'id' in s])
            else:
                last_id = 0
            ...
            logger.info(f"Added {len(new_samples)} new samples to {content_type} training data")
    ...
    def generate_training_template(self, content_type: str = 'gm', count: int = 5) -> str:
        """生成训练样本模板（方便用户填写）
    ...
    def get_learning_dashboard(self) -> str:
        ...
        if recent_updates:
            for update in recent_updates:
                ...
                lines.append(f"   {date}: {update['type']} (+{update['samples_added']} 样本)")
        else:
            lines.append("   （无训练记录）")
...
# 便捷函数
def add_training_samples(content_type: str, samples: List[Dict], notes: str = "") -> dict:
    """快速添加训练样本"""
...
def generate_training_template(content_type: str = 'gm', output_file: str = None) -> str:
    """生成训练模板"""
    system = ContinuousLearningSystem()
