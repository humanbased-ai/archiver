...
class MockLLMForPython(LLMInterface):
    @property
    def model_name(self) -> str:
        return "mock_llm_python_v1"
    ...
    def generate_response(self, system_prompt: str, user_prompt: str, json_mode: bool = False) -> Any:
        ...
        # print(f"\n--- MockLLM (Python) Called ---")
        # print(f"User Prompt (first 100): {user_prompt[:100]}...")
        # print(f"JSON Mode: {json_mode}")
        # print("------------------------------------\n")

        if "python code snippet" in user_prompt.lower() and "user profile" in user_prompt.lower() and not json_mode:
            # Mock response for Python code generation
            return MockResponse("""
import json # Example of allowing a common import, though instructions say self-contained

def validate_data(data):
    if not isinstance(data, dict):
        return {"is_valid": False, "reason": "Input data must be a dictionary."}
    ...
    if not (email and isinstance(email, str) and '@' in email and '.' in email.split('@')[-1]):
        return {"is_valid": False, "reason": "'email' must be a valid-looking email address."}
    
    # Check for presence of all required fields explicitly
    required_fields = ['name', 'age', 'email']
    for field in required_fields:
        if field not in data:
            return {"is_valid": False, "reason": f"Missing required field: '{field}'."}
            
    return {"is_valid": True, "reason": "Data is valid according to Python script."}
""")
        return MockResponse(None, error=f"MockLLM (Python): Unhandled prompt. User prompt: {user_prompt[:100]}...")

def run_python_example():
    print("===== Running Python Handler Example =====")
    mock_llm = MockLLMForPython()
    ...
    artifact_type_to_generate = "python"
    ...
    # Avoid printing the full artifact if it's long code
